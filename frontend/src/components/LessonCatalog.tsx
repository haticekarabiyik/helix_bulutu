...
export default function LessonCatalog() {
  ...
  useEffect(() => {
    // Stellar Kontrat Client'ını initialize et
    // Kontrat ID'si (gerçek uygulamada bu değer config'ten gelir)
    const LESSON_CONTRACT_ID = "CAHHXRCUWYBHZX5BBDVHG4GJFDCJMRFMFKGSDQSWUDJB6HPVUIPD2ZUEJ";
    const client = new LessonContractClient(LESSON_CONTRACT_ID);
    ...
    <div className="p-6 bg-slate-900/80 border border-slate-700 rounded-3xl shadow-xl backdrop-blur-sm">
      <div className="mb-5 flex flex-col gap-3">
        <div className="inline-flex items-center gap-2 rounded-full bg-emerald-500/10 px-4 py-2 text-sm text-emerald-200">
          Ücretli Eğitim Paketi
        </div>
        <div>
          <h2 className="text-xl font-bold text-white">Özel Ders Konuları</h2>
          <p className="text-slate-400 text-sm leading-6">
            Coulomb elektrik, bir boyutlu ivmeli hareket ve benzeri konular için ücretli ders içerikleri.
            Cüzdanla bağlanıp dilediğiniz konuyu satın alabilirsiniz.
          </p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 mb-6">
        {LESSONS.map((lesson) => (
          <button
            ...
            className={`rounded-3xl border p-4 text-left transition ${selectedLesson?.id === lesson.id
          ...
          >
            <div className="flex items-center justify-between gap-3 mb-3">
              ...
              <span className="text-xs uppercase tracking-[0.2em] text-slate-400">{lesson.price}</span>
            </div>
            <p className="text-slate-400 text-sm leading-5">{lesson.description}</p>
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-950/80 p-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-slate-400 mt-2">{selectedLesson.description}</p>
            </div>
            {isOwned(selectedLesson.id, selectedInstructor.id) ? (
              <button
                type="button"
                disabled
                className="rounded-2xl px-4 py-3 text-sm font-semibold bg-slate-700 text-slate-200"
              >
                Satın Alındı ✓
              </button>
            ) : (
              <button
                ...
                className="rounded-2xl px-4 py-3 text-sm font-semibold bg-emerald-500 text-slate-950 hover:bg-emerald-600 transition"
              >
                {convertTLtoXLM(selectedInstructor.price)} XLM ile Satın Al
              </button>
            )}
          </div>
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="space-y-4">
              <div className="rounded-3xl border border-white/10 bg-black/30 p-5 text-slate-300">
                <p className="font-semibold text-slate-100">Ders İçeriği</p>
              ...
              <div className="rounded-3xl border border-white/10 bg-slate-950/80 p-5">
                <p className="font-semibold text-white">Eğitmen Seçimi</p>
                <div className="mt-4 space-y-3">
                  {selectedLesson.instructors.map((instructor) => (
                    <button
                      ...
                      className={`w-full rounded-3xl border p-4 text-left transition ${selectedInstructor.id === instructor.id
                        ? "border-emerald-500 bg-emerald-500/10"
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="text-sm font-semibold text-emerald-300">{getInstructorDisplayPrice(instructor.price)}</span>
                      </div>
              ...
              <div className="rounded-3xl border border-white/10 bg-slate-950/80 p-5">
                <p className="font-semibold text-white">Ders Testleri</p>
                <p className="text-slate-400 text-sm mt-2">Her videodan sonra kolay, orta, zor ve aşırı zor testlere çalışın.</p>
            ...
            <div className="space-y-4">
              <div className="rounded-3xl border border-white/10 bg-black/30 p-5 text-slate-300">
                <p className="text-sm text-slate-400">Öğrenci E-Postası</p>
                <input
                  type="email"
                  value={studentEmail}
              ...
              <div className="rounded-3xl bg-slate-950/80 p-5 border border-white/10">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="rounded-2xl bg-emerald-500/10 px-3 py-2 text-sm text-emerald-300">{selectedInstructor.role}</span>
                </div>
                <p className="mt-3 text-sm text-slate-400">{selectedInstructor.experience}</p>
              </div>

              <div className="rounded-3xl bg-black/30 p-5 text-slate-300 border border-white/10">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-sm text-slate-400">İnceleme</p>
          ...
          <div className="mt-5 rounded-3xl bg-slate-950/80 p-4 text-sm text-slate-400">
            {status === "connected" && address ? (
              <p>Cüzdanınız: {address.slice(0, 6)}...{address.slice(-6)}</p>
            ) : (
              <p>Satın alma işlemi için cüzdanınıza bağlanın.</p>
            )}
          </div>
        </div>
      )}
